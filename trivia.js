...
if (isMobile) {
  ...
  questionImage.addEventListener('click', () => {
    const currentQuestion = questions[currentQuestionIndex];
    if (currentQuestion.type === 'imageChange' && !questionImage.classList.contains('locked')) {
      const secondImageSrc = questionImage.dataset.secondImage;
      if (secondImageSrc) {
        questionImage.src = secondImageSrc;
        questionImage.classList.add('locked'); // bloqueamos nuevos clics

        setTimeout(() => {
          if (currentQuestion.image) {
            questionImage.src = currentQuestion.image;
          }
          questionImage.classList.remove('locked'); // liberamos después de 3s
        }, 3000);
      }
    }
  });
} else {
  // --- Versión para PC (mouse) ---
